...
const resumeVisibilityBlock = source.slice(source.indexOf('function resumeFromVisibility'), source.indexOf('function showMessage'));
assert(resumeVisibilityBlock.includes('if (document.hidden) return;') && resumeVisibilityBlock.includes('startLoop();') && !resumeVisibilityBlock.includes('if (!pauseStartedAt) return;'), 'visibility resume must restart the canvas even when the browser did not deliver a matching pause event');
