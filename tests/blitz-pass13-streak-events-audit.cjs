...
assert.match(engine, /STREAK_EVENT_THRESHOLDS = Object\.freeze\(\[3, 5, 8, 12, 15\]\)/,
  'Pass 3 must define distinct streak events at 3, 5, 8, 12, and 15');
...
assert.match(engine, /streak-event-3.*streak-event-5.*streak-event-8.*streak-event-12.*streak-event-15/s,
...
assert.match(engine, /const fullCount = \(\{ 3: 6, 5: 12, 8: 20, 12: 32, 15: 48 \}\)\[threshold\]/,
  'milestone particles must escalate across the unified ladder');
assert.match(engine, /const holdMs = \(\{ 3: 400, 5: 520, 8: 650, 12: 780, 15: 900 \}\)\[threshold\]/,
  'milestone holds must escalate without becoming slow');
assert.match(engine, /const eventScale = \(\{ 3: 1\.02, 5: 1\.03, 8: 1\.04, 12: 1\.05, 15: 1\.06 \}\)\[threshold\]/,
