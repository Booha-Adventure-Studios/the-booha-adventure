...
assert(source.includes("getWandererImage(filename, { required: true })"), 'current-room Wanderer images must be protected at cache admission');
...
assert(source.includes('protectedBytes'), 'Karasuki overlay must expose protected decoded bytes');
assert(source.includes('overBudget'), 'Karasuki overlay must expose protected-over-budget state');
