...
assert(utsuroba.includes("const danceArmsUpImg = makeUtsurobaDeferredImage('./assets/img/booha_ghost_dance_arms_up.png');"), 'dance arms-up art must not request at boot');
assert(utsuroba.includes("const danceSwayImg   = makeUtsurobaDeferredImage('./assets/img/booha_ghost_dance_sway.png');"), 'dance sway art must not request at boot');
assert(utsuroba.includes("const danceWaveImg   = makeUtsurobaDeferredImage('./assets/img/booha_ghost_dance_wave.png');"), 'dance wave art must not request at boot');
...
assert(sw.includes('booha-assets-2026-392'), 'service-worker asset cache must include the current Utsuroba loading bump');
