...
assert(!unlock.includes('GRIMMERGLEN_BUILD_READY'),
  'the shipped world must not retain the temporary build gate');
...
assert(unlockFn.includes('return isWeeklyWorldGateOpen();'),
  'regular players must pass the shared weekly world gate');
...
for (const pageName of profilePages) {
  ...
  assert(source.includes('id="pnet-grimmerglen"'),
    `${pageName} must expose the gated Grimmerglen profile tab`);
  assert(source.includes('id="pnet-grimmerglen" hidden'),
    `${pageName} must hide Grimmerglen before the weekly gate`);
  assert(source.includes('isWeeklyWorldGateOpen()'),
    `${pageName} must use the shared weekly world gate`);
  assert(source.includes('tab.hidden = !open;'),
    `${pageName} must reveal Grimmerglen only after the weekly gate opens`);
}

assert(runtime.includes("stage.addEventListener('mousemove'"),
  'DEV mode must track the live mouse position');
assert(runtime.includes('mouse:'),
  'DEV readout must label the live mouse coordinates');
assert(runtime.includes('booha:'),
  'DEV readout must retain Booha coordinates alongside the mouse');
assert(runtime.includes('stage.addEventListener(\'mouseleave\''),
  'DEV mode must clear the mouse readout when the cursor leaves the stage');
