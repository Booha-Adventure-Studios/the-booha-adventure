...
// Pass 20H: short popup/click sounds use the shared WebAudio palette. BGM,
// looped tracks, and Muenba's reading-cue context remain separate.
...
['popupOpen', 'popupClose', 'buttonPress', 'skeletonClose', 'mischiefReward', 'slimeClick', 'ghostError', 'lockedRattle', 'phantomCancel'].forEach(name => {
...
['popupOpen', 'popupClose', 'buttonPress', 'ghostError', 'phantomCancel', 'mischiefReward'].forEach(name => {
...
assert(serviceWorker.includes('booha-pages-2026-372'), 'service-worker page cache must include the current popup pass bump');
