...
echo "[74B/76] Muenba Pass 5 touch-target audit"
if node tests/muenba-pass5-touch-target-audit.cjs >/dev/null 2>&1; then
  ok "Muenba final mobile touch-target contracts pass"
else
  bad "Muenba Pass 5 touch-target audit failed"
fi
...
echo "[90E/95] Grimmerglen Pass 5 orientation audit"
if node tests/grimmerglen-pass5-orientation-audit.cjs >/dev/null 2>&1; then
  ok "Grimmerglen phone orientation and safe-area contracts pass"
else
...
echo "[90I/95] Utsuroba Pass 3 performance audit"
if node tests/utsuroba-pass3-performance-audit.cjs >/dev/null 2>&1; then
  ok "Utsuroba touch DPR and modal frame-loop contracts pass"
else
...
echo "[blitz] Pass 5 performance audit"
if node tests/blitz-pass5-performance-audit.cjs >/dev/null 2>&1; then
  ok "Blitz minimal node, layout, and deferred-loading contracts pass"
else
...
echo "[blitz] Pass 6 color/feel audit"
if node tests/blitz-pass6-color-feel-audit.cjs >/dev/null 2>&1; then
  ok "Blitz curriculum personalities and reward finish contracts pass"
else
  bad "Blitz Pass 6 color/feel audit failed"
fi

echo "[blitz] Pass 7 shared theme-source audit"
if node tests/blitz-pass7-theme-source-audit.cjs >/dev/null 2>&1; then
  ok "Blitz Pass 7 shared theme-source audit passed"
else
  bad "Blitz Pass 7 shared theme-source audit failed"
fi

echo "[blitz] Pass 2 personality audit"
if node tests/blitz-pass8-personality-audit.cjs >/dev/null 2>&1; then
  ok "Blitz Pass 2 personality audit passed"
else
...
echo "[blitz] Pass 4 feedback audit"
if node tests/blitz-pass10-feedback-audit.cjs >/dev/null 2>&1; then
  ok "Blitz Pass 4 feedback audit passed"
else
...
echo "[blitz] Pass 2 correct-feedback audit"
if node tests/blitz-pass14-correct-feedback-audit.cjs >/dev/null 2>&1; then
  ok "Blitz Pass 2 correct-feedback audit passed"
else
  bad "Blitz Pass 2 correct-feedback audit failed"
fi

echo "[blitz] Pass 3 final-energy audit"
if node tests/blitz-pass15-final-energy-audit.cjs >/dev/null 2>&1; then
  ok "Blitz Pass 3 final-energy audit passed"
else
  bad "Blitz Pass 3 final-energy audit failed"
fi

echo "[blitz] Pass 4 wrong-recovery audit"
if node tests/blitz-pass16-wrong-recovery-audit.cjs >/dev/null 2>&1; then
  ok "Blitz Pass 4 wrong-recovery audit passed"
else
  bad "Blitz Pass 4 wrong-recovery audit failed"
fi

echo "[blitz] Pass 5 start-card audit"
if node tests/blitz-pass17-start-card-audit.cjs >/dev/null 2>&1; then
  ok "Blitz Pass 5 start-card audit passed"
else
  bad "Blitz Pass 5 start-card audit failed"
fi

echo "[blitz] Pass 6 audio-variation audit"
if node tests/blitz-pass18-audio-variation-audit.cjs >/dev/null 2>&1; then
  ok "Blitz Pass 6 audio-variation audit passed"
else
  bad "Blitz Pass 6 audio-variation audit failed"
fi

echo "[blitz] Pass 7 motion audit"
if node tests/blitz-pass19-motion-audit.cjs >/dev/null 2>&1; then
  ok "Blitz Pass 7 motion audit passed"
else
  bad "Blitz Pass 7 motion audit failed"
fi

echo "[blitz] Pass 8 extra-juice audit"
if node tests/blitz-pass20-juice-audit.cjs >/dev/null 2>&1; then
  ok "Blitz Pass 8 extra-juice audit passed"
else
  bad "Blitz Pass 8 extra-juice audit failed"
fi

echo "[blitz] Pass 1 contrast audit"
if node tests/blitz-pass21-contrast-audit.cjs >/dev/null 2>&1; then
  ok "Blitz Pass 1 contrast audit passed"
else
  bad "Blitz Pass 1 contrast audit failed"
fi

echo "[blitz] Pass 2 streak-meter audit"
if node tests/blitz-pass22-streak-meter-audit.cjs >/dev/null 2>&1; then
  ok "Blitz Pass 2 streak-meter audit passed"
else
  bad "Blitz Pass 2 streak-meter audit failed"
fi

echo "[blitz] Pass 3 particle audit"
if node tests/blitz-pass23-particle-audit.cjs >/dev/null 2>&1; then
  ok "Blitz Pass 3 particle audit passed"
else
...
echo "[blitz] Pass 5 correct-contrast audit"
if node tests/blitz-pass25-correct-contrast-audit.cjs >/dev/null 2>&1; then
  ok "Blitz Pass 5 correct-contrast audit passed"
else
  bad "Blitz Pass 5 correct-contrast audit failed"
fi

echo "[blitz] Pass 1 furigana-layout audit"
if node tests/blitz-pass26-furigana-layout-audit.cjs >/dev/null 2>&1; then
  ok "Blitz Pass 1 furigana-layout audit passed"
else
  bad "Blitz Pass 1 furigana-layout audit failed"
fi

echo "[blitz] Pass 2 streak-container audit"
if node tests/blitz-pass27-streak-container-audit.cjs >/dev/null 2>&1; then
  ok "Blitz Pass 2 streak-container audit passed"
else
  bad "Blitz Pass 2 streak-container audit failed"
fi

echo "[blitz] Pass 3 fire-wallpaper audit"
if node tests/blitz-pass28-fire-wallpaper-audit.cjs >/dev/null 2>&1; then
  ok "Blitz Pass 3 fire-wallpaper audit passed"
else
  bad "Blitz Pass 3 fire-wallpaper audit failed"
fi

echo "[blitz] wrong-retry BGM audit"
if node tests/blitz-pass29-wrong-retry-bgm-audit.cjs >/dev/null 2>&1; then
  ok "Blitz wrong-retry BGM audit passed"
else
...
echo "[blitz] visibility-lifecycle audit"
if node tests/blitz-pass36-visibility-lifecycle-audit.cjs >/dev/null 2>&1; then
  ok "Blitz visibility-lifecycle audit passed"
else
...
echo "[blitz] performance-tier audit"
if node tests/blitz-pass38-performance-tier-audit.cjs >/dev/null 2>&1; then
  ok "Blitz performance-tier audit passed"
else
...
if node tests/games-audio-polish-audit.cjs >/dev/null 2>&1; then
  ...
  bad "Nine-engine audio polish audit failed"
fi
