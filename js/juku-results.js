...
(function () {
  ...
  function summarizeSection(rec) {
    ...
    const correct = items.filter(it => it && it.ok).length;
  ...
  function computeReport(week) {
    ...
    // Prove-it: committed re-asks inside mixed. Denominator is items
    // actually presented-and-committed; zero → null (never 0/0).
    const mixedItems = (week.sections && week.sections.mixed &&
                        week.sections.mixed.items) || [];
    const pv = mixedItems.filter(it => it && it.proveIt);
    const proveIt = pv.length
      ? { correct: pv.filter(it => it.ok).length, total: pv.length }
      : null;
    ...
    return {
      ...
      scoredSectionCount: included.length,
    ...
    if (s.missing) {
      return `
        <div class="juku-res-row missing">
          ...
          <span class="juku-res-frac">うけてない</span>
        </div>`;
    }
  ...
  function render(taskEl, res, slot) {
    ...
    const totalHTML = report.total === null
      ? `<p class="juku-res-total none">けいさん できません</p>`
