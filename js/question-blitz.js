...
window.QuestionBlitz = (() => {
  ...
  const PALETTES = {
    pb: {
      bg: ['#00001a','#000828','#00102e','#000520','#000a24'],
      accent: '#00cfff',
      accent2: '#ffffff',
    ...
    br: {
      bg: ['#000a1a','#001228','#001830','#000d22','#001026'],
      accent: '#3d8eff',
      accent2: '#aaccff',
    ...
    bc: {
      bg: ['#05001a','#0a0028','#100030','#070020','#0c0026'],
      accent: '#a855f7',
      accent2: '#d8b4fe',
  ...
  const SCOLDS = [
    ...
    { jp: '次は質問を読んでみて。', hira: 'つぎはしつもんをよんでみて。', en: 'Maybe read the questions next time.' },
    { jp: 'あ、惜しい！…全然。', hira: 'あ、おしい！…ぜんぜん。', en: 'Oh, so close! ...Not really.' },
    { jp: '質問、ちゃんと読んだ？', hira: 'しつもん、ちゃんとよんだ？', en: 'Did you actually read the question?' },
  ...
  function getBestTime(curr) {
    try {
      ...
      return data?.meta?.questionBlitz?.[curr] ?? null;
    } catch { return null; }
  }
  ...
  function saveBestTime(curr, ms) {
    try {
      ...
      if (!data.meta.questionBlitz) data.meta.questionBlitz = {};
      const existing = data.meta.questionBlitz[curr];
      if (existing === null || existing === undefined || ms < existing) {
        data.meta.questionBlitz[curr] = ms;
  ...
  function startGame(allCards, curr, weekNumber, palette, monthSlug) {
    ...
    function showWin(ms, curr, palette, overlay, winScreen, monthSlug, weekNumber) {
      ...
      winScreen.querySelector('#qb-win-time-val').style.color = palette.timerColor;
      winScreen.querySelector('#qb-win-time-val').style.textShadow =
        `0 0 32px ${palette.glow}, 0 0 64px ${palette.glow}`;
      winScreen.querySelector('#qb-win-record-msg').textContent =
        isRecord ? '新記録！ NEW RECORD!' : '';
      winScreen.querySelector('#qb-win-best-val').textContent =
        isRecord ? '' : `ベスト: ${fmtTime(best)}`;
