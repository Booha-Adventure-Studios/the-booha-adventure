...
/* ══════════════════════════════════════════════════════════════
   TIERS  — speaking-focused, different from vocab/sentence speed
   ══════════════════════════════════════════════════════════════ */
const TIERS = [
...
/* ══ HEADER ══ */
.stw-header{ text-align:center; padding:.6rem 3rem .6rem; width:100%; }
.stw-curriculum{
...
/* curriculum-specific card backgrounds */
[data-curriculum="br"] .stw-jp-card{
  ...
  box-shadow:0 10px 40px rgba(0,0,0,.35), 0 0 60px rgba(167,139,250,.08);
}
[data-curriculum="bc"] .stw-jp-card{
  ...
  box-shadow:0 10px 40px rgba(0,0,0,.5), 0 0 30px rgba(0,240,255,.08);
}
...
.stw-mode-badge{
  ...
  padding:4px 14px; border-radius:99px; margin-bottom:1rem;
...
.stw-jp-card.listening .stw-mode-dot{ background:#ef4444; animation:stwDotBlink .7s ease-in-out infinite; }
...
.stw-jp-word{
  ...
  line-height:1.1; color:var(--game-ink);
...
.stw-hira{
  margin-top:.5rem; font-family:var(--game-font-jp);
  font-size:clamp(14px,2.8vw,20px); color:var(--game-muted);
}
[data-curriculum="pb"] .stw-hira{ color:rgba(58,26,46,.55); }

/* ── card state: dancing (correct) ── */
.stw-jp-card.dancing .stw-jp-word{
  animation:stwDance .6s ease-in-out infinite alternate;
}
@keyframes stwDance{
...
/* ── card listening pulse ── */
.stw-jp-card.listening{
  border-color:rgba(239,68,68,.6) !important;
  animation:stwListenPulse 1s ease-in-out infinite;
}
@keyframes stwListenPulse{
  0%,100%{ box-shadow:0 0 0 4px rgba(239,68,68,.15), 0 10px 40px rgba(0,0,0,.35); }
  50%{     box-shadow:0 0 0 10px rgba(239,68,68,.25), 0 10px 50px rgba(0,0,0,.45); }
}

/* ══════════════════════════════════════════════════════════════
   ANSWER DISPLAY (mic mode — shows what SR heard)
   ══════════════════════════════════════════════════════════════ */
.stw-heard-box{
...
[data-curriculum="pb"] .stw-heard-box{
  background:#fff; border-color:#e8d4ff; color:rgba(58,26,46,.45);
  box-shadow:0 3px 0 #eeddff;
}
[data-curriculum="pb"] .stw-heard-box.heard-correct{
  border-color:#22c55e; color:#22c55e; background:#f0fff4; box-shadow:0 3px 0 #86efac;
}
...
.stw-mic-btn svg{
  width:36px; height:36px; fill:#fff; display:block;
}
.stw-mic-btn:hover{
  transform:translateY(-3px) scale(1.07);
  box-shadow:0 0 44px rgba(167,139,250,.7), 0 8px 0 rgba(80,20,160,.5), 0 14px 28px rgba(0,0,0,.35);
}
.stw-mic-btn:active{ transform:scale(.9); }
.stw-mic-btn:disabled{
  opacity:.35; pointer-events:none; transform:none !important; box-shadow:none !important;
}
...
.stw-hint{
  ...
  color:var(--game-muted); text-align:center; margin-bottom:.6rem;
  line-height:1.45;
}
...
[data-curriculum="bc"] .stw-ios-choice{
  background:linear-gradient(145deg,rgba(0,220,255,.1),rgba(0,100,200,.07));
  border-color:rgba(0,200,240,.25);
}
[data-curriculum="pb"] .stw-ios-choice{
  background:#ffffff; color:#2a1020; border:2.5px solid #cc88ff;
  box-shadow:0 5px 0 #ddb8ff, 0 8px 18px rgba(180,100,255,.1);
}
...
[data-curriculum="pb"] .stw-ios-choice.ios-correct{
  background:#f0fff4 !important; border-color:#22c55e !important; color:#22c55e !important;
  box-shadow:0 0 0 4px rgba(34,197,94,.2), 0 0 28px rgba(34,197,94,.3), 0 4px 0 #86efac !important;
}
...
[data-curriculum="pb"] .stw-ios-choice.ios-wrong{
  background:#fff5f5 !important; border-color:#ef4444 !important; color:#ef4444 !important;
  box-shadow:0 0 0 4px rgba(239,68,68,.18), 0 0 24px rgba(239,68,68,.22), 0 4px 0 #fca5a5 !important;
}
...
.stw-next-btn{
  ...
  font-family:var(--game-font-title);
  font-size:clamp(18px,3.5vw,24px); letter-spacing:.1em;
  padding:16px 28px; border:none; border-radius:999px;
  cursor:pointer; position:relative; overflow:hidden;
  background:linear-gradient(135deg, #22c55e, #16a34a);
  color:#fff; font-weight:900;
  box-shadow:0 0 28px rgba(34,197,94,.6), 0 5px 0 rgba(10,90,30,.55), 0 10px 24px rgba(0,0,0,.3);
  animation:stwNextGlow 1.4s ease-in-out infinite;
  -webkit-tap-highlight-color:transparent; margin:0 auto;
  transition:transform .15s;
}
@keyframes stwNextGlow{
  0%,100%{ box-shadow:0 0 28px rgba(34,197,94,.6), 0 5px 0 rgba(10,90,30,.55), 0 10px 24px rgba(0,0,0,.3); }
  50%{     box-shadow:0 0 55px rgba(34,197,94,1),  0 5px 0 rgba(10,90,30,.55), 0 14px 28px rgba(0,0,0,.4); }
}
.stw-next-btn::after{
  content:''; position:absolute; top:-50%; left:-80%; width:48%; height:200%;
  background:linear-gradient(105deg,transparent 28%,rgba(255,255,255,.35) 50%,transparent 72%);
  transform:skewX(-16deg); animation:stwNextShimmer 2s ease infinite;
  pointer-events:none;
}
@keyframes stwNextShimmer{ 0%{ left:-80%; } 60%,100%{ left:150%; } }
.stw-next-btn:hover{ transform:translateY(-3px) scale(1.04); }
.stw-next-btn:active{ transform:scale(.96); }
...
.stw-help-btn:hover{
  ...
  box-shadow:0 0 14px rgba(167,139,250,.4); transform:scale(1.08);
}
...
[data-curriculum="bc"] .stw-help-btn:hover{ border-color:#00ddff; color:#00ddff; box-shadow:0 0 14px rgba(0,200,220,.4); }
...
[data-curriculum="bc"] .stw-modal{ border-color:#00ddff; box-shadow:0 0 48px rgba(0,200,220,.25), 0 24px 48px rgba(0,0,0,.5); }
...
.stw-modal-header{
  ...
  background:linear-gradient(135deg,rgba(167,139,250,.12),rgba(120,60,220,.07));
...
.stw-modal-title{
  font-family:var(--game-font-title); font-size:clamp(20px,4vw,26px); letter-spacing:.06em;
  color:#a78bfa; text-shadow:0 0 16px rgba(167,139,250,.5);
}
[data-curriculum="bc"] .stw-modal-title{ color:#00ddff; text-shadow:0 0 16px rgba(0,200,220,.5); }
[data-curriculum="pb"] .stw-modal-title{ color:#cc88ff; text-shadow:none; }
.stw-modal-title-jp{ font-family:var(--game-font-jp); font-size:clamp(12px,2vw,15px); color:var(--game-muted); margin-top:4px; }
[data-curriculum="pb"] .stw-modal-title-jp{ color:rgba(58,26,46,.55); }
...
[data-curriculum="pb"] .stw-how-jp{ color:rgba(58,26,46,.55); }
.stw-modal-close{
...
.stw-modal-close:active{ transform:scale(.96); }
...
.stw-start-card{
  max-width:460px; width:100%; border-radius:32px; overflow:hidden;
...
[data-curriculum="bc"] .stw-start-card{ background:#030810; border-color:rgba(0,240,255,.5); box-shadow:0 0 60px rgba(0,240,255,.12), 0 24px 48px rgba(0,0,0,.6); }
.stw-start-header{
  padding:1.5rem 1.4rem .9rem;
  background:linear-gradient(135deg,color-mix(in srgb,var(--game-primary) 12%,transparent),color-mix(in srgb,var(--game-secondary) 7%,transparent));
  border-bottom:1px solid var(--game-border);
}
.stw-start-title{
  font-family:var(--game-font-title); font-size:clamp(24px,5.5vw,44px);
...
[data-curriculum="bc"] .stw-start-title{
  background:linear-gradient(90deg,#00f0ff,#4455ff,#aa00ff,#00f0ff);
  background-size:220% auto; -webkit-background-clip:text; background-clip:text;
}
[data-curriculum="pb"] .stw-start-title{
  background:linear-gradient(90deg,#ff6eb4,#cc88ff,#44ccff,#ffcc44,#ff6eb4);
  background-size:220% auto; -webkit-background-clip:text; background-clip:text;
}
.stw-start-subtitle{
  font-family:var(--game-font-jp); font-size:clamp(11px,1.9vw,14px);
  color:var(--game-muted); margin-top:5px;
}
...
.stw-start-num{
  ...
  background:linear-gradient(135deg,var(--game-primary),var(--game-secondary));
  color:#000; font-family:var(--game-font-title); font-size:clamp(12px,2vw,15px); font-weight:900;
  display:flex; align-items:center; justify-content:center; flex-shrink:0;
  box-shadow:0 0 9px color-mix(in srgb,var(--game-primary) 42%,transparent);
}
...
[data-curriculum="pb"] .stw-start-jp{ color:rgba(58,26,46,.5); }
.stw-start-btn{
  display:block; width:calc(100% - 2.8rem); margin:1.2rem 1.4rem 1.5rem;
  font-family:var(--game-font-title); font-size:clamp(17px,3.5vw,24px); letter-spacing:.1em;
  padding:15px 22px; border:none; border-radius:999px;
  cursor:pointer; position:relative; overflow:hidden;
  background:linear-gradient(135deg,var(--game-primary),var(--game-secondary));
  color:#000; font-weight:900;
  box-shadow:0 0 30px color-mix(in srgb,var(--game-primary) 50%,transparent), 0 5px 0 color-mix(in srgb,var(--game-primary) 38%,#000), 0 10px 22px rgba(0,0,0,.3);
...
/* ══════════════════════════════════════════════════════════════
   RESULTS PANEL  — purple/violet palette for Say the Word
   ══════════════════════════════════════════════════════════════ */
.stw-results{
...
/* Top bar — purple sweep for Say the Word */
.stw-results::before{
  content:''; position:absolute; top:0; left:0; right:0; height:5px;
  background:linear-gradient(90deg,#a78bfa,#ff2288,#ffcc00,#22ddff,#a78bfa);
  background-size:220% auto; animation:stwRainbow 2.4s linear infinite;
}
[data-curriculum="bc"] .stw-results::before{
  background:linear-gradient(90deg,#00f0ff,#4455ff,#aa00ff,#00f0ff);
  background-size:220% auto;
}
[data-curriculum="pb"] .stw-results::before{
  background:linear-gradient(90deg,#ff6eb4,#cc88ff,#44ccff,#ffcc44,#ff6eb4);
  background-size:220% auto;
}
.stw-results::after{
  ...
  background:
    radial-gradient(circle at 25% 75%, color-mix(in srgb,var(--stw-tier-color,#a78bfa) 12%,transparent) 0%,transparent 50%),
    radial-gradient(circle at 75% 25%, color-mix(in srgb,var(--game-secondary) 8%,transparent) 0%,transparent 50%);
}
.stw-res-inner{ position:relative; z-index:1; }
.stw-res-score{
  font-family:var(--game-font-title); font-size:clamp(62px,16vw,98px);
  line-height:1; color:var(--stw-tier-color,#a78bfa);
  text-shadow:0 0 28px var(--stw-tier-color,#a78bfa); margin-bottom:4px;
  animation:stwScorePop .55s cubic-bezier(.22,.8,.36,1) .3s both;
}
@keyframes stwScorePop{
...
.stw-res-label{
  font-family:var(--game-font-title); font-size:clamp(24px,5.5vw,38px);
  color:var(--stw-tier-color,#a78bfa); margin-bottom:10px; letter-spacing:.05em;
  text-shadow:0 0 18px color-mix(in srgb,var(--stw-tier-color,#a78bfa) 55%,transparent);
  animation:stwFadeUp .4s ease .52s both;
}
.stw-res-divider{
  width:60px; height:3px; border-radius:99px;
  background:linear-gradient(90deg,#a78bfa,#ff2288,#22ddff);
  margin:0 auto 12px; opacity:.6; animation:stwFadeUp .4s ease .56s both;
}
...
/* ══════════════════════════════════════════════════════════════
   BUILD HTML
   ══════════════════════════════════════════════════════════════ */
const modeLabel     = isIOS ? 'CHOOSE MODE / えらぼう' : 'MIC MODE / マイクモード';
...
<div class="stw-wrap" id="stw-main-wrap">
  ...
  <div class="stw-jp-card" id="stw-jp-card" style="width:100%">
    <div class="stw-mode-badge" id="stw-badge">
      <div class="stw-mode-dot" id="stw-badge-dot"></div>
  ...
  <!-- Heard box (mic mode only) -->
  <div class="stw-heard-box" id="stw-heard" style="${isIOS ? 'display:none' : ''}">
  ...
  <!-- iOS 4-CHOICE GRID -->
  ${isIOS ? `<div class="stw-ios-grid" id="stw-ios-grid"></div>` : ''}

  <div class="stw-hint" id="stw-hint">
    ${hintTextEN}<br>${hintTextJP}
  </div>

  <!-- NEXT BUTTON — hidden until correct -->
  <button class="stw-next-btn" id="stw-next" style="display:none">
    NEXT ▶ / 次へ
  </button>
...
<div class="stw-modal-overlay" id="stw-modal-overlay">
  <div class="stw-modal" role="dialog" aria-modal="true">
    ...
    <div class="stw-modal-body">
      <div class="stw-how-step">
        <div class="stw-how-num">1</div>
        <div>
      ...
      <div class="stw-how-step">
        <div class="stw-how-num">2</div>
        <div>
          <div class="stw-how-en">Tap the microphone button.</div>
          <div class="stw-how-jp">マイクボタンをタップ。</div>
        </div>
      </div>
      <div class="stw-how-step">
        <div class="stw-how-num">3</div>
        <div>
          <div class="stw-how-en">Say the English word out loud clearly!</div>
          <div class="stw-how-jp">英語をはっきり声に出して言おう！</div>
        </div>
      </div>
      `}
      <div class="stw-how-step">
        <div class="stw-how-num">${isIOS ? 4 : 4}</div>
        <div>
...
<div class="stw-start-overlay" id="stw-start-overlay">
  <div class="stw-start-card">
    ...
    <div class="stw-start-body">
      <div class="stw-start-step">
        <div class="stw-start-num">1</div>
        <div>
      ...
      <div class="stw-start-step">
        <div class="stw-start-num">2</div>
        <div>
          <div class="stw-start-en">${startStep3EN}</div>
          <div class="stw-start-jp">${startStep3JP}</div>
        </div>
      </div>
      <div class="stw-start-step">
        <div class="stw-start-num">3</div>
        <div>
...
/* ══════════════════════════════════════════════════════════════
   iOS: BUILD 4 CHOICES
   The correct word + 3 distractors, shuffled.
   ══════════════════════════════════════════════════════════════ */
...
function handleIosPick(btn, en) {
  ...
  if (correct) {
    ...
    /* lock other tiles */
    Array.from(iosGrid.children).forEach(b => {
...
/* ══════════════════════════════════════════════════════════════
   MIC MODE: SPEECH RECOGNITION (Android / Desktop)
   ══════════════════════════════════════════════════════════════ */
...
/* ══════════════════════════════════════════════════════════════
   RESULTS  — dispatches booha:gameEnd for say_word
   ══════════════════════════════════════════════════════════════ */
...
/* showCard() called by START button only */
